...
| POST | `/run` | 手動執行一輪監控 |
| POST | `/import-session` | 寫入 session（`local-test.js` 使用） |
...
# 或上傳至正式 Worker（預設 import URL 見 package.json `local:test:prod`）
npm run local:test:prod
```

流程：本機瀏覽器手動登入 Candy House → 按 Enter → 上傳 cookies + `localStorage` 至 `/import-session`。
...
npm run local:test             # 登入並匯入 session 至本機
npm run local:test:prod        # 登入並匯入 session 至正式 Worker
...
- **Candy House WebSocket 逾時**：session 可能仍有效；確認 DevTools → Network → WS 是否有 `PubedCompanyDevice` 且工寮 UUID 帶 `CHSesame2Status`。定期執行 `npm run local:test:prod` 更新 session。
